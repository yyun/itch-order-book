...
enum class itch_t {
  ...
  ADD_ORDER = 'A',         // 36
  ADD_ORDER_MPID = 'F',
  EXECUTE_ORDER = 'E',  // 订单成交信息
  EXECUTE_ORDER_WITH_PRICE = 'C',
  REDUCE_ORDER = 'X',   // Modified after partial cancellation 部撤
  DELETE_ORDER = 'D',   // 撤单
  REPLACE_ORDER = 'U',  // 撤单然后替换成新的订单
  TRADE = 'P',          // 成交明细
  CROSS_TRADE = 'Q',
  BROKEN_TRADE = 'B',
...
static uint64_t read_eight(char const *src) { return be64toh(*(uint64_t const *)src); }
...
static uint32_t read_four(char const *src) { return be32toh(*(uint32_t const *)src); }
static uint16_t read_two(char const *src) { return be16toh(*(uint16_t const *)src); }
...
struct itch_message<MSG::ADD_ORDER> {
  ...
  BUY_SELL const buy;
  static itch_message parse(char const *ptr) {
    return add_order_t(read_timestamp(ptr + 5), read_oid(ptr + 11), read_price(ptr + 32), read_qty(ptr + 20), read_locate(ptr + 1),
                       BUY_SELL(*(ptr + 19)));
  }
};
...
struct itch_message<MSG::ADD_ORDER_MPID> {
  ...
  static itch_message parse(char const *ptr) { return itch_message(add_order_t::parse(ptr)); }
};
using execute_order_t = itch_message<MSG::EXECUTE_ORDER>;
template <>
struct itch_message<MSG::EXECUTE_ORDER> {
  itch_message(oid_t __oid, timestamp_t __t, qty_t __q, uint16_t __s) : oid(__oid), timestamp(__t), qty(__q), stock_locate(__s) {}
  oid_t const oid;
  timestamp_t const timestamp;
  qty_t const qty;
  uint16_t const stock_locate;
...
struct itch_message<MSG::EXECUTE_ORDER_WITH_PRICE> {
  ...
  static itch_message parse(char const *ptr) { return itch_message(execute_order_t::parse(ptr)); }
};
using order_reduce_t = itch_message<MSG::REDUCE_ORDER>;
template <>
struct itch_message<MSG::REDUCE_ORDER> {
  itch_message(oid_t __o, timestamp_t __t, qty_t __q) : oid(__o), timestamp(__t), qty(__q) {}
  oid_t const oid;
  timestamp_t const timestamp;
  qty_t const qty;
  static itch_message parse(char const *ptr) { return itch_message(read_oid(ptr + 11), read_timestamp(ptr + 5), read_qty(ptr + 19)); }
};
...
struct itch_message<MSG::DELETE_ORDER> {
  ...
  oid_t const oid;
  timestamp_t const timestamp;
  static itch_message parse(char const *ptr) { return itch_message(read_oid(ptr + 11), read_timestamp(ptr + 5)); }
};
using order_replace_t = itch_message<MSG::REPLACE_ORDER>;
template <>
struct itch_message<MSG::REPLACE_ORDER> {
  itch_message(oid_t __old_oid, oid_t __new_oid, qty_t __q, price_t __p) : oid(__old_oid), new_order_id(__new_oid), new_qty(__q), new_price(__p) {}
  oid_t const oid;
